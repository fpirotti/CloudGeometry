...
void KdTree::arma_range_nearest_neighbors(const CoordPoint& point, double r,
                                     arma::mat * result) {
  KdNode temp;
  ...
  result->set_size(range_result.size(), 3);
  int rn=0;
  // copy over result
  for (std::vector<size_t>::iterator i = range_result.begin();
       i != range_result.end(); ++i) {
...
//--------------------------------------------------------------
// recursive function for range search in subtree under *node*.
// Stores result directly in Armadillo matrix (Francesco Pirotti)
//--------------------------------------------------------------
void KdTree::arma_range_search(const CoordPoint& point, kdtree_node* node,
                          double r, arma::mat &range_result) {
  double curdist = distance->distance(point, node->point);
  if (curdist <= r) {
     arma::rowvec new_row = arma::conv_to<arma::rowvec>::from(node->point);
    range_result = arma::join_vert(range_result, new_row);

  }
  if (node->loson != NULL && this->bounds_overlap_ball(point, r, node->loson)) {
    arma_range_search(point, node->loson, r, range_result);
  }
  if (node->hison != NULL && this->bounds_overlap_ball(point, r, node->hison)) {
    arma_range_search(point, node->hison, r, range_result);
  }
}
